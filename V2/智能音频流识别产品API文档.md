...
| speakers       | json_array         | Y            | <p>声网合流录制审核情况下，数美会每隔1s调用一次声网接口获取当前时间段说话人信息（内容包括说话人id和音量大小，见下方speakers参数说明），在每次的音频流片段结果回调中返回该信息</p><p>普通流或其他RTC流录制方式下，该字段返回值为空</p>                                                                                                                                                                                                                  |
...
| **参数名称**      | **类型**    | **是否必选** | **说明**                                                                                                                                                                                                                                                                                                                                      |
| :---------------- | :---------- | :----------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| errorCode         | int         | 是           |<p>状态码</p><p>3001：流地址访问失败，例如资源HTTP状态码404、403</p><p>3002：流数据无效，例如“Invalid data found when processing input”</p><p>3003：流不存在，例如zego返回197612错误码</p><p>3004：流未返回音频数据</p><p>3005：拉流token无效或过期，建议使用新token重新开启审核，例如声网token过期或者trtc usersig无效</p>|
| streamTime       | int         | 否           | 流结束后最后一次返回，代表送审时长，如有间隔审核逻辑时，和流真实时长可能不一致  |

detail.speakers数组中每一项具体参数如下：

| 参数名称   | 类型 | 是否必选 | 说明                                                                         |
| :--------- | :--- | :------- | :--------------------------------------------------------------------------- |
| uid      | int  | Y        | <p>说话人id</p> |
| volumn | int  | Y        | <p>说话人音量大小，是相对值，具体含义参考声网接口</p>|
